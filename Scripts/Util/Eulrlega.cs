...
namespace Parang.Util
{
    public static class Eulrlega
    {
        ...
        private static readonly string Dilimiter = "^";
        ...
        public static bool IsKorean(this string text)
        {
            Regex regex = new Regex(@"[ㄱ-ㅎ|ㅏ-ㅣ|가-힣]");
        ...
        {
            // 비어있는 스트링 영문 혹은 숫자로만 구성되어 있는 스트링
            ...
            {
                var token1 = string.Concat(Dilimiter, Formats[i][0]);
                var token2 = string.Concat(Dilimiter, Formats[i][1]);
            ...
            return string.Format(str.Replace(Dilimiter, ""), values);
        }

        private static string ProcessFormat(string str, string target, string[] eulrl, object[] values)
        {
            var token = string.Concat(Dilimiter, target);
